    ...
    if (this.checkInput()) {
      ...
      axios.put(HttpRequestPath + userRegistrationApi, data).then(response => {
        if (response.data == emailErr) {
          this.setState({
            notFilledErr: true,
            emailRegisteredErr: strings.emailRegistered
          });
        } else if (response.data == usernameErr) {
          this.setState({
            notFilledErr: true,
            usernameRegisteredErr: strings.usernameRegistered
          });
      ...
      buttons: [
        {
          ...
          onClick: () => {
            ...
            this.props.history.push("/registration/camera");
          }
        },
        {
          label: strings.no,
      ...
      (
        <div className="boxRegistration">
          ...
          {this.state.notFilledErr ? (
            <Message
              error
              header={strings.tryAgain}
              list={[
                this.state.notFilledFieldsErr,
                this.state.usernameRegisteredErr,
                this.state.emailRegisteredErr
              ]}
            />
          ) : null}
